...
class FaceGeometryApp:
    def __init__(self, master):
        ...
        self.angle_frame = tk.LabelFrame(self.frame_controls, text="测量角度", padx=5, pady=5)
        ...
        try:
            ...
            def fmt(v):
                try:
                    ...
                    return "--"
            line1 = f"下半面宽度: {fmt(lw_px)} px"
            if lw_mm is not None:
                line1 += f" ({fmt(lw_mm)} mm)"
            self.metrics_text.insert(tk.END, line1 + "\n")
            self.metrics_text.insert(tk.END, f"左侧角度: {fmt(side.get('left'))}°\n")
            self.metrics_text.insert(tk.END, f"右侧角度: {fmt(side.get('right'))}°\n")

...
# 鼻梁点 (dlib 的鼻梁点 27-30) - 这些点是中轴线上最核心的
...
# --- 新增：使用线性回归计算人脸中线 (X,Y反转) ---
# 收集所有用于拟合中线的关键点
midline_fit_points = []

# 优先选择那些更靠近中线且受表情影响较小的点
...
# 确定人脸中线的 Y 范围 (保持不变，根据眉毛和下巴来定)
midline_y_top_limit = min(p[1] for p in (pts[19], pts[24])) - 50 # 眉毛最上方的点再往上一些
midline_y_bottom_limit = pts[8][1] + 50 # 下巴最底部的点再往下一些


# 根据拟合出的直线方程 (x = slope_prime * y + intercept_prime)，计算中线的两个端点
...
for p_fit in midline_fit_points:
    cv2.circle(out, tuple(p_fit), 5, (255,0,255), -1) # 使用和中线相同的颜色
...
# --- 可视化点：新中线和相关关键点 ---
cv2.imshow("Midline and Fitted Keypoints", out.copy()) # 使用 out.copy() 以免影响后续绘制
cv2.waitKey(0)
# --- 可视化结束 ---
...
lower_skin = np.array([5, 53, 92], dtype="uint8")  # 调低V值以适应较暗肤色
upper_skin = np.array([18, 140, 225], dtype="uint8")
...
lower_skin_ycbcr = np.array([93, 135, 80], dtype="uint8")
upper_skin_ycbcr = np.array([220, 165, 116], dtype="uint8")
...
kernel = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))  # 椭圆形内核
